...
type UpdatePayload = Partial<
  Pick<ParameterDto, 'name' | 'unitOfMeasure' | 'factor' | 'offset' | 'isActive'>
>

export const TelemetryService = {
  async listParameters({ q = '', limit = 10, cursor = null }: ListParams) {
    const byName = (p: ParameterDto) => p.name.toLowerCase().includes(q.toLowerCase())
  ...
  async listStations(params: {
    ...
    const filtered = mockStations
      .filter((s) => {
        ...
        const byStatus =
          status === 'todos' ? true : status === 'Ativo' ? s.isActive : !s.isActive
        const byActivity =
          activity === 'todos'
            ? true
            : s.lastReadAt
  ...
  async listStationMeasurements(params: {
    ...
    if (parameterId !== 'todos') rows = rows.filter((m) => m.parameter.id === parameterId)
