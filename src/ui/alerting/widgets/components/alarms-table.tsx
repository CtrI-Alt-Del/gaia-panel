...
const getIcon = (iconName: string) => {
  const IconComponent =
    ICON_MAP[iconName as keyof typeof ICON_MAP] || ThermometerSun;
  return <IconComponent className="w-4 h-4" />;
};

const getIconColor = (iconName: string) => {
  return (
    ICON_COLOR_MAP[iconName as keyof typeof ICON_COLOR_MAP] || "text-orange-500"
  );
};
...
const AlarmRow = ({ alarm, onEdit, onDelete }: AlarmRowProps) => {
  return (
              ...
              {TABLE_HEADERS.map((header) => (
                <th
                  ...
                  className={`px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider ${
                    CENTERED_HEADERS.includes(header as any)
                      ? "text-center"
                      : "text-left"
                  }`}
                >
