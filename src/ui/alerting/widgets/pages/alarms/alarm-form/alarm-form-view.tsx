    ...
    <>
      <form onSubmit={onSubmit} className='space-y-6'>
        <div className='space-y-3'>
          ...
          {selectedStationId ? (
            <div className='flex items-center justify-between p-3 border rounded-md bg-gray-50'>
              ...
              <Button type='button' variant='outline' size='sm' onClick={() => setIsSheetOpen(true)}>
                <Edit className='w-4 h-4 mr-2' />
                Alterar
              </Button>
            </div>
          ) : (
            <Button type='button' variant='outline' className='w-full justify-start text-gray-500' onClick={() => setIsSheetOpen(true)}>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          ...
          <Controller name='operation' control={control} render={({ field }) => (
            <div className='space-y-3'>
              ...
              {errors.operation && <p className='text-sm text-red-600'>{errors.operation.message}</p>}
            </div>
          )} />
        </div>
        ...
        <div className='space-y-3'>
            ...
            <Input id='threshold' type='number' step='any' placeholder='Digite o valor...' {...register('threshold')} className={errors.threshold ? 'border-red-500' : ''} />
            {errors.threshold && <p className='text-sm text-red-600'>{errors.threshold.message}</p>}
        </div>

        <div className='flex justify-end gap-3 pt-4 border-t'>
          <Button type='button' variant='outline' onClick={onCancel} disabled={isSubmitting}>Cancelar</Button>
          <Button type='submit' disabled={isSubmitting}>{isSubmitting ? 'Criando...' : 'Criar Alarme'}</Button>
        </div>
      </form>
      ...
      <ParameterSelectorSheet
        ...
        onSelect={(station, parameter) => {
          setValue('stationId', station.id, { shouldValidate: true })
          setValue('parameterId', parameter.id, { shouldValidate: true })
