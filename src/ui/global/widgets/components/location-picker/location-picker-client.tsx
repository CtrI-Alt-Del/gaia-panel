...
export const LocationPickerClient = ({
  ...
  const [mapCenter, setMapCenter] = useState<[number, number]>([latitude || -23.5505, longitude || -46.6333])
  ...
  const geocodeAddress = useCallback(
    async (addressToGeocode: string) => {
      if (!addressToGeocode.trim() || addressToGeocode === `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`) {
        return
      }
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="space-y-4">
        ...
        {isLoadingCoords && (
          <p className="text-xs text-muted-foreground">Atualizando coordenadas...</p>
        )}
      </div>
