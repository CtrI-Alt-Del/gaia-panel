...
type Props = {
  ...
  onReadAlertButtonClick: (alertId: string) => void
}

export const AlertsPopoverView = ({
  alerts,
  isOpen,
  unreadAlertCount,
  alertDialogRef,
  onOpenChange,
  onReadAlertButtonClick,
}: Props) => {
  const { formatDateTime } = useDateTimeProvider()
  const hasAlerts = alerts.length > 0
  const newAlertsBadgeLabel = hasAlerts ? `${unreadAlertCount} novos` : 'Sem novos'
  ...
  return (
    <>
      <AlertDialog
        ref={alertDialogRef}
        title='Novos alertas críticos detectados'
        description='Foram identificados novos alertas críticos. Deseja visualizá-los agora?'
        confirmText='Ver alertas'
        onConfirm={() => onOpenChange(true)}
        variant='destructive'
      />
              ...
              alerts.map((alert, index) => {
                return (
                  <article
                    ...
                    className={cn(
                      'rounded-2xl border border-zinc-300 bg-card/60 px-4 py-3 transition-colors hover:border-border hover:bg-card/80',
                    )}
                  >
