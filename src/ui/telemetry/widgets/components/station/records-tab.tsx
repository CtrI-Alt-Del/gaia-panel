    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-stone-200">
        <div className="flex items-center justify-between p-4 border-b border-stone-200">
          <h2 className="text-lg font-medium">Registros de Medição</h2>
        </div>
        <table className="w-full text-sm">
          <thead className="bg-stone-50 text-left">
            <tr>
              ...
              <th className="p-3 font-medium text-stone-700">Data da Leitura</th>
            </tr>
          </thead>
          <tbody>
            {loading && (
              <tr>
                <td colSpan={3} className="p-6 text-center text-stone-500">Carregando…</td>
              </tr>
            )}
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan={3} className="p-6 text-center text-stone-500">Nenhum registro encontrado</td>
              </tr>
            )}
            {!loading && rows.map(r => (
              <tr key={r.id} className="border-t border-stone-100 hover:bg-stone-50">
                <td className="p-3 font-medium text-stone-900">{r.parameter.name}</td>
                <td className="p-3 text-stone-600">{r.value} {r.parameter.unitOfMeasure}</td>
                <td className="p-3 text-stone-600">{formatDateTime(r.measuredAt)}</td>
              </tr>
            ))}
          </tbody>
        </table>
