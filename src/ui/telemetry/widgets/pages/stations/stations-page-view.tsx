    ...
    <section className="container mx-auto px-4 py-2">
      <header className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-stone-600">Filtros por UID, nome e status</p>
        </div>
      </header>
        ...
        <table className="w-full text-sm">
          <thead className="bg-stone-50 text-left">
            <tr>
              ...
              <th className="p-3 text-center font-medium text-stone-700">Ações</th>
            </tr>
          </thead>
          <tbody>
            {loading && (
              <tr><td colSpan={7} className="p-6 text-center text-stone-500">Carregando…</td></tr>
            )}
            {!loading && rows.length === 0 && (
              <tr><td colSpan={7} className="p-6 text-center text-stone-500">Nenhuma estação encontrada</td></tr>
            )}
            {!loading && rows.map((s) => (
              <tr key={s.id} className="border-t border-stone-100 hover:bg-stone-50">
                <td className="p-3 font-medium text-stone-900">{s.UID}</td>
                <td className="p-3">
                  <Link to={`/stations/${s.id}`} className="text-blue-600 hover:text-blue-800 hover:underline font-medium">{s.name}</Link>
                </td>
                <td className="p-3 text-stone-600">{s.latitude.toFixed(4)}, {s.longitude.toFixed(4)}</td>
                <td className="p-3 text-stone-600">{s.parameters.length} parâmetros</td>
                <td className="p-3">
                  <StatusPill active={s.isActive || false} />
                </td>
                <td className="p-3 text-stone-600">{s.lastReadAt ? timeAgo(s.lastReadAt) : '—'}</td>
                <td className="p-3 text-right">
                  <div className="flex gap-2 justify-center">
                    ...
                    <button
                      ...
                      title={s.isActive ? "Desativar estação" : "Ativar estação"}
                    >
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-stone-600">
        <span>Mostrando {rows.length ? `${fromTo.from}–${fromTo.to}` : 0} de {total} resultados</span>
